...
const ChatElement: FC<ChatElementProps> = ({ id, name, focused, setFocused }) => {
  ...
  return (
    ...
    >
      <Link
        href={`/?${id}`}
        className={`relative flex items-center break-all rounded-md hover:cursor-pointer ${focused === id ? "bg-gray-800 hover:bg-gray-800 pr-[60px]" : "bg-gray-900 hover:bg-[#2A2B32]"} gap-3 p-3`}
      >
        ...
        <div className="relative flex-1 overflow-hidden max-h-6 break-all text-ellipsis text-gray-200">
          {name}
          <div className={`absolute inset-y-0 right-0 w-8 bg-gradient-to-l ${focused === id ? "from-gray-800" : "from-gray-900"}`}></div>
        </div>
